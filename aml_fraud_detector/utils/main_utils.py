...
def align_features_with_schema(
    df: pd.DataFrame,
    schema: "FeatureSchema",
    log_context: Optional[str] = None,
) -> pd.DataFrame:
    """
    Unified feature alignment function - all inference paths must go through this.

    Performs:
    1. Column name normalization (lower, replace spaces/dots)
    2. Derived feature generation (e.g., day from timestamp)
    3. Missing column filling with defaults
    4. Extra column dropping
    5. Fixed column ordering
    6. Type casting

    This is the single source of truth for feature alignment.
    Flask, Streamlit, PredictionPipeline all call this function.
    """
    context = f"[{log_context}] " if log_context else ""

    logging.info(
        f"{context}Aligning features: input_cols={df.columns.tolist()}, "
        f"expected_cols={schema.model_input_columns}"
    )

    aligned_df = schema.align_features(df)

    logging.info(
        f"{context}Features aligned: output_cols={aligned_df.columns.tolist()}, "
        f"dtypes={dict(aligned_df.dtypes)}"
    )

    return aligned_df


def load_feature_schema(schema_path: str) -> "FeatureSchema":
    """
    Load feature schema from disk and validate it's complete.
    """
    schema = load_object(schema_path)

    if not hasattr(schema, 'model_input_columns') or not schema.model_input_columns:
        raise ValueError(
            f"Loaded feature schema from {schema_path} is missing model_input_columns. "
            f"Schema may be corrupted or generated by an incompatible version."
        )

    logging.info(
        f"Loaded feature schema v{schema.schema_version} "
        f"(source: {schema.schema_source}) from {schema_path}"
    )
    logging.info(f"Schema model input columns: {schema.model_input_columns}")
