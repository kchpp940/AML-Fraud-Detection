...
class ArtifactValidator:
    ...
    def _sha256_digest(self, path: str, chunk_size: int = 1 << 20) -> str:
        sha = hashlib.sha256()
    ...
    def _validate_required_files(self, errors: List[str]) -> None:
        critical_files = {
            "model.pkl": self.model_path,
            "preprocessor.pkl": self.preprocessor_path,
            "feature_metadata.json": self.feature_metadata_path,
            "model_metadata.json": self.model_metadata_path,
        }
    ...
    def validate(self, strict: bool = True) -> ValidationReport:
        ...
        self._validate_required_files(errors)
        if errors:
