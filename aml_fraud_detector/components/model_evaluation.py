...
class ModelEvaluation:
    ...
    def _try_mlflow_evaluate(self, model, X_test, y_test):
        try:
            import mlflow
            ...
            with mlflow.start_run():
                ...
                if tracking_url_type_store != "file":
                    mlflow.sklearn.log_model(
                        model, "model",
                        registered_model_name="RandomForestBestModel"
                    )
                else:
                    mlflow.sklearn.log_model(model, "model", signature=signature)
        except ImportError as e:
            logging.warning(
                f"mlflow not available, skipping mlflow tracking: {e}"
            )
            predictions = model.predict(X_test)
            (precision, recall, f1, cm) = self.eval_metrics(y_test, predictions)
            print(f"Precision: {precision}")
    ...
    def initiate_model_evaluation(self, train_array, test_array):
        try:
            ...
            self._try_mlflow_evaluate(model, X_test, y_test)
